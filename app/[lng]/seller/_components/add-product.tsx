...
const AddProduct = ({ categories }: { categories: Category[] }) => {
  ...
  // Extend product type to allow subcategoryId to be string or Subcategory object
  type ProductType = {
    _id: string;
    title: string;
    description: string;
  ...
  const { open, setOpen, product, setProduct } = useProduct() as {
    open: boolean;
  ...
  // category tanlanganda subcategories set qilish
  useEffect(() => {
    if (selectedCategory) {
  ...
  async function onSubmit(values: z.infer<typeof productSchema>) {
    ...
    setIsLoading(true);
    const subcategoryId =
      typeof values.subcategoryId === "object" &&
      values.subcategoryId !== null &&
      "._id" in values.subcategoryId
        ? (values.subcategoryId as { _id: string })._id
        : values.subcategoryId;

    let res;
    if (product?._id) {
      res = await updateProduct({ ...values, subcategoryId, id: product._id });
    } else {
      res = await createProduct({ ...values, subcategoryId });
    }
  ...
  // 1. Avval product.category ni set qilamiz
  useEffect(() => {
    if (product?._id) {
  ...
  // 2. Subcategories tayyor bo'lganda form.reset qilamiz
  // ...existing code...
  useEffect(() => {
    if (product?._id && selectedCategory) {
      const foundCategory = categories.find(
        (cat) => cat._id === selectedCategory
      );

      let subSlug = "";
      if (product.subcategoryId && foundCategory) {
        if (
          typeof product.subcategoryId === "object" &&
          product.subcategoryId !== null
        ) {
          subSlug = product.subcategoryId.slug;
        } else {
          // Agar string bo'lsa, slug bo'lishi kerak
          subSlug = product.subcategoryId;
        }
      }
  ...
  }, [product, selectedCategory, categories, form]);
  // ...existing code...
